...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Decide the outcome of a shard from its test export and its queue record: fail on any failed test "
                    "without a known issue or failing on something else than its known issue, on any skipped test (a skip "
                    "means a configuration method failed, usually the container start), on a Maven run that exited with an "
                    "error and on a claimed class that exported no result."
    )
    parser.add_argument("--export-dir", default="target/test-export", type=Path)
    ...
    for result_file in sorted(args.export_dir.rglob("result.json")):
        ...
        ticket = str(known_issue.get("ticket") or "")
        if status == "PASSED":
            passed += 1
            if ticket:
                fixed_candidates.append(f"{test} ({ticket})")
        elif status == "FAILED" and ticket and known_issue.get("causeMatched") is True:
            known_issues.append(f"{test} ({ticket})")
        elif status == "FAILED" and ticket:
            failure = " ".join(str(result.get("errorMessage") or "").split())
            blocking.append(f"{test}: FAILED, but not on its known issue {ticket} — the failure does not contain "
                            f"'{known_issue.get('failsWith') or ''}': {failure}")
        else:
    ...
    if blocking:
        raise SystemExit("The shard has failed or skipped tests without a known issue, tests failing on something else than "
                         "their known issue, or a class run that produced no results")
