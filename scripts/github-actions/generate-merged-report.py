...
def collect_from_test_export(input_root: Path, output_dir: Path) -> tuple[list[TestRecord], set[Path]]:
...
- `attachments` — failure screenshot and downloaded files; the failure video is not copied into the report,
  it stays in the shard artifact.
...
def write_step_summary(records: list[TestRecord], title: str, build: str, summary_path: str | None, info: RunInfo) -> str:
    ...
    lines += [
        ...
        "failure videos stay in the shard artifacts.",
    ]
    text = "\n".join(lines) + "\n"
    if summary_path:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Merge the shard test exports into one HTML report, per-test debug bundles and a job summary."
    )
    ...
    records, _ = collect_from_test_export(args.input_root, args.output_dir)
