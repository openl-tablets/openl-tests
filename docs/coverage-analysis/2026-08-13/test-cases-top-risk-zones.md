...
Scope: openl-tests (TestNG + Playwright). 28 cases across 7 zones — user-scenario oriented, each zone carries its negative checks, no case inflation. Assertions state the EXPECTED behavior from the tickets/spec; if actual differs, file a bug and keep the case as a known-failing regression linked to it (never weaken the assertion).
