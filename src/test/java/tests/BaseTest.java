...
public abstract class BaseTest implements ITest {
    ...
    public void beforeMethod(ITestResult result) {
        ...
        TestExportUtil.startTest(result, getTestName());
        ...
        TestExportUtil.recordApplicationInfo(applicationImage.get(), applicationInfo);
    }
        ...
        try {
            ...
            try {
                try {
                    ...
                    TestExportUtil.finishTest(result);
                }
            } finally {
    ...
    private void attachDebugArtifacts(ITestResult result, boolean videoRecorded) {
        ...
        boolean keepDebugArtifacts = result.getStatus() != ITestResult.SUCCESS || DebugArtifactUtil.isDebugArtifactsOnSuccessEnabled();

        boolean traceAttached = DebugArtifactUtil.attachTrace(testName, keepDebugArtifacts);

        if (failed) {
            DebugArtifactUtil.attachScreenshotOnFailure(testName);
            if (!traceAttached) {
                DebugArtifactUtil.attachPageContent("Page Content at Failure");
            }
            DebugArtifactUtil.attachExecutionInfo();
        }

        if (failed && videoRecorded) {
            DebugArtifactUtil.attachVideoOnFailure(testName);
        }

        if (keepDebugArtifacts) {
            File appLog = LogsUtil.saveAppLogs(AppContainerPool.get());
            TestExportUtil.recordAttachment("Application LOG", "INFO", appLog);
        }
    }
