...
OpenL Tablets pull requests dispatch this workflow with `source_sha` and `pr_number` after Quick Build publishes matching images tagged `pr-<number>-<head SHA>`. Those inputs must be supplied together and must match `application_version`. Runs for the same pull request share a concurrency group, so a new head commit cancels the obsolete run. After the report is published, the workflow uses the `E2E_APP_ID` and `E2E_APP_PRIVATE_KEY` organization credentials to post `success` or `failure` to the `openl-tests` commit status on the source commit. Standalone manual runs leave both inputs empty and do not post a cross-repository status.
...
The report is also published to GitHub Pages, so it opens in the browser without downloading the artifact. Standalone runs continue to publish under `runs/<run id>/`; `scripts/github-actions/publish-report-pages.py` keeps the latest 15 and regenerates the site index at https://openl-tablets.github.io/openl-tests/ with the date, build, results and tests commit of each run. A run dispatched for an OpenL Tablets pull request instead replaces `pr-<number>/`, so concurrent pull requests cannot overwrite each other and a status link remains stable across new commits. Every update is a single orphan commit pushed with `--force-with-lease`, so pruned reports do not stay in Git history and the branch does not grow. The job summary links to the published report.
