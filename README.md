...
- `TestGitLfsGitHubStorage`, `TestGitLfsGitLabStorage`, `TestGitLfsBitbucketStorage`: the same scenario (`GitLfsExternalStorageTest`) against the LFS storage of a real provider. The design repository stays in Gitea; its `.lfsconfig` points LFS at the provider (`helpers.service.LfsProvider`, `ExternalLfsService`), so only LFS objects reach the provider, never a branch or a commit. GitHub hands out presigned S3 URLs for uploads, like Bitbucket, and rejects what S3 does not accept (EPBDS-16711: chunked uploads get HTTP 501); GitLab takes uploads on gitlab.com.
