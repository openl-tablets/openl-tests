...
`TestGitLfsDesignRepository` checks LFS against the Gitea container, whose own LFS server accepts any upload. `TestGitLfsGitHubStorage` checks it against GitHub LFS, which hands out presigned S3 URLs and rejects uploads S3 does not accept (EPBDS-16711: chunked uploads get HTTP 501). The design repository stays in Gitea; its `.lfsconfig` points LFS at `https://github.com/<this repository>.git/info/lfs`, so only LFS objects reach GitHub, no branch or commit. The class is `@DedicatedWorkflow` and runs only in the `OpenL LFS Tests` workflow (`.github/workflows/openl-lfs.yml`), started by hand with the Studio image tag (`latest` by default). The workflow token is the only credential: it uploads the fixture object, is the Gitea password and is the password Studio sends to both. Before artifacts are uploaded, `scripts/github-actions/check-secret-leaks.py` searches every file for the token: raw, inside any base64 such as Basic credentials, split across lines, and inside zip and gzip archives at any depth; the merged report and the job summary are checked the same way. When the token is found nothing is uploaded, written to the job summary or published. Otherwise the workflow uploads the shard artifact `openl-lfs` and the merged report `test-report-merged`, and publishes the report to GitHub Pages next to the regression runs; the site keeps the last 15 runs of both workflows together, so each LFS run pushes out the oldest report. `org.testcontainers` and `com.github.dockerjava` log at `info`, because at `debug` they print the environment of the Studio container, where the token is the repository password. Locally the test takes `GITHUB_TOKEN`, or `gh auth token` when it is not set. LFS objects cannot be deleted from GitHub, so each run leaves about 20 KB in the LFS storage of the repository.
