...
The workflow runs on demand and on every pull request to `main`: when it is opened, reopened, marked ready for review and on every push to it. Draft pull requests, pull requests from forks and Dependabot pull requests are not run, because their token cannot write the queue refs. A new push cancels the run of the previous push of the same pull request; a run started by hand is never cancelled. The pull request run is informational: a failed test turns the check red, but no check is required, so the pull request can still be merged.

The `application_version` input is the ghcr.io tag of the OpenL images (`webstudio:<tag>` and `ws:<tag>-all`); it defaults to `latest`, the newest published build, and a pull request run always uses `latest`. The "Discover tests" job resolves both tags once with `scripts/github-actions/resolve-images.py` (`docker buildx imagetools inspect`) and every shard runs the image by digest (`webstudio@sha256:...`), so all shards test the same build even when `latest` is republished during the run. A wrong tag fails the job within seconds, so a typo no longer costs a full run of tests that cannot start their container. The report and the Pages index show the tag the run asked for, the digest it was resolved to, the openl-tablets revision and the build time of the image, and mark `latest` as a moving tag; a pull request run is labelled with its number and title.

GitHub Actions does not read the TestNG suite files. `scripts/github-actions/discover-tests.py` scans `src/test/java` and queues every non-abstract class that has `@Test` methods, except classes annotated `@LocalOnly(reason = "...")` (`configuration.annotations.LocalOnly`), which are meant for local runs only: special infrastructure, manual data, exploratory checks. A new test class is therefore part of the GitHub regression the moment it is committed, and nothing has to be added to a suite for it. A concrete class whose `@Test` methods are all inherited from a base class is found through its `extends` clause. The four classes of the `*_regression` suites (`TestZippedProjects`, `TestPreconfigProjects`, `TestStudioCentralGroupPolicyBundle`, `TestStudioCentralGroupRatingClaim`) carry `@LocalOnly` because they need local project sets, JDK 25 or the Genesis central repositories; every other test class is queued, except classes annotated `@DedicatedWorkflow(workflow = "...", reason = "...")` (`configuration.annotations.DedicatedWorkflow`), which run only in the workflow they name. Because the queue is keyed by run id and run attempt, a re-run of the workflow starts with a fresh queue.
...
All Git LFS tests run only in the `OpenL LFS Tests` workflow (`.github/workflows/openl-lfs.yml`), started by hand with the Studio image tag (`latest` by default); they are `@DedicatedWorkflow`, so the regression neither queues them nor accepts them in its `tests` input. Four shards take the four classes from the same queue the regression uses and run them in parallel (a fast shard may take two classes and another none); the `Merge test report` job builds one report from their artifacts. The classes are:

- `TestGitLfsDesignRepository`: LFS on the Gitea container itself, whose LFS server accepts any upload.
- `TestGitLfsGitHubStorage`, `TestGitLfsGitLabStorage`, `TestGitLfsBitbucketStorage`: the same scenario (`GitLfsExternalStorageTest`) against the LFS storage of a real provider. The design repository stays in Gitea; its `.lfsconfig` points LFS at the provider (`helpers.service.LfsProvider`, `ExternalLfsService`), so only LFS objects reach the provider, never a branch or a commit. GitHub hands out presigned S3 URLs for uploads, like Bitbucket, and rejects what S3 does not accept (EPBDS-16711: Studio built with JGit 7.8.0.202609011348-openl sent chunked uploads and got HTTP 501; fixed in openl-tablets 825e4093ae); GitLab takes uploads on gitlab.com.

Studio sends one login and password to Gitea and to the LFS server, so the Gitea owner gets the provider's login (`x-access-token`, `openl-lfs-test`, `x-bitbucket-api-token-auth`) and the provider token as password. Credentials and repositories come from the environment:

| Provider | Repository | Token |
|---|---|---|
| GitHub | this repository | `GITHUB_TOKEN` (the workflow token; locally `gh auth token` when it is not set) |
| GitLab | secret `LFS_GITLAB_REPO` | secret `LFS_GITLAB_TOKEN` (personal access token, `write_repository`) |
| Bitbucket | secret `LFS_BITBUCKET_REPO` | secret `LFS_BITBUCKET_TOKEN` (Atlassian API token, repository read and write) |

The repository URLs are internal paths, so they are secrets too: the artifacts, the report, the job summary and Pages name the variable (`${LFS_GITLAB_REPO}`), never the value. In the console of the job, where a file cannot be rewritten, the values rely on GitHub masking and show as `***`: a step right after checkout fails when a secret is empty and masks the Basic credentials and every URL form that GitHub would not mask by itself. The tests write variable names instead of secret values in every message they produce (`helpers.utils.SecretText`). In every shard, `scripts/github-actions/redact-secrets.py` replaces any remaining value with its variable name in every file the shard uploads, inside zip and gzip archives such as Playwright traces too. Then `scripts/github-actions/check-secret-leaks.py` searches those files, and in the merge job the merged report and the job summary, for every token (raw, URL-encoded, inside any base64 such as Basic credentials, split across lines) and every form of the repository URLs, inside archives at any depth. When anything is found in a shard, that shard uploads nothing; when anything is found in the merged report, it is neither uploaded, written to the job summary nor published. The report is published only when it holds all four classes; otherwise the merge job fails and names the missing ones. Otherwise each shard uploads its artifact `openl-tests-lfs-NN`, and the merge job uploads the merged report `test-report-merged` and publishes the report to GitHub Pages next to the regression runs; the site keeps the last 30 runs of both workflows and of the pull request runs together, so each run pushes out the oldest report. `org.testcontainers` and `com.github.dockerjava` log at `info`, because at `debug` they print the environment of the Studio container, where the token is the repository password. LFS objects cannot be deleted from the providers, so each run leaves about 20 KB in the LFS storage of each repository.
...
The report is also published to GitHub Pages, so it opens in the browser without downloading the artifact: `scripts/github-actions/publish-report-pages.py` pushes every run to the `gh-pages` branch under `runs/<run id>/` as a single orphan commit (`--force-with-lease`, so pruned reports do not stay in the git history and the branch never grows), keeps the last 30 runs, pull request runs included, and regenerates the site index at https://openl-tablets.github.io/openl-tests/ with the date, build, results and tests commit of each run; the job summary links to the published report.
