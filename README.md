...
All Git LFS tests run only in the `OpenL LFS Tests` workflow (`.github/workflows/openl-lfs.yml`), started by hand with the Studio image tag (`latest` by default); they are `@DedicatedWorkflow`, so the regression neither queues them nor accepts them in its `tests` input. The workflow runs, one after another:
